package com.ydeatimes.channelk.web.util;
...
public class ImageTransferUtil {
	...
	public static final int ORIGIN_QUALITY = 0;
	...
	public void save(MultipartFile file, Image img) throws IllegalStateException, IOException {		
		File origin = new File(img.getSavePathWithQuality(ORIGIN_QUALITY));
		...
		for (int i=0; i<len; i++) {
			...
			if(bi!=null){
				File resize = new File(img.getSavePathWithQuality(quality[i]));
				ImageIO.write(bi, img.getFileExt(), resize);
				files.add(resize);
			}
		}
	...
	public void saveOne(MultipartFile file, Image img) throws IllegalStateException, IOException {
		File origin = new File(img.getSavePathWithQuality(ORIGIN_QUALITY));
		file.transferTo(origin);
		files.add(origin);
	}
