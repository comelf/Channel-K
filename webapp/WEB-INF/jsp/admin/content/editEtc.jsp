...
<body>
        <div id="page-wrapper">
            <div class="container-fluid">
				...
				<form:form modelAttribute="etcContent" role="form" id="form" action="/admin/etccontent/edit" method="post" >
	                <form:input path="id" type="text" cssClass="hidden" />
	                <div class="row">
	                		<div class="col-md-12">
	                			<div class="image-block">
		                			<label>썸네일 (필수)</label>
		                			<img src="<c:if test="${not empty etcContent.thumbnail}">${etcContent.thumbnail.getUrlPathWithQuality(0) }</c:if>" width="400" class="">
	                		...
	                		<div class="col-md-6">
		                		<div class="form-group">
		                			<label>연관 Content</label>
		                			<form:select path="content_id" cssClass="form-control">
		                				<option value="0">선택하세요</option>
		                				<c:forEach var="info" items="${contentInfoList}">
		                					<option value="${info.id}" <c:if test="${info.id eq etcContent.info.id}">selected="selected"</c:if>>${info.title}</option>
		                				</c:forEach>
								    </form:select>
		                		</div>
		                	</div>
		                		...
		                		<div class="form-group">
								    ...
								    <form:select path="status_text" cssClass="form-control">
		                				...
		                				<c:forEach var="status" items="${contentStatusList}">
		                					<option value="${status.text}" <c:if test="${status.text eq etcContent.status.text}">selected="selected"</c:if>>${status.text}</option>
		                				</c:forEach>
								    </form:select>
		                		</div>
						    </div>
